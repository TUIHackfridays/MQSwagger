...
// register listener to process interrution to close connnection log
...
const createReqResQueue = (ch, name, options) => {
  const requestQueue = createSimpleQueue(ch, name,
    _.defaults(options.request || {}));

  const responseQueue = createSimpleQueue(ch,
    options ? options.responseQueue : '',
    _.defaults(options.response || {}, {
      exclusive: true
    }));
  ...
  return Promise.join(requestQueue, responseQueue, consumer, (req, res) => ({
    ...
    request: (msg, opts) => {
      ...
      }).timeout(options.timeout || 5000, corr + 'response timed out');
